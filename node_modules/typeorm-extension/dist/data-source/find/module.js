...
async function findDataSource(context) {
    ...
    if (context.directory) {
        if (node_path_1.default.isAbsolute(context.directory)) {
            ...
            directory = (0, utils_1.safeReplaceWindowsSeparator)(context.directory);
        }
    }
    const lookupPaths = [];
    ...
    if (!(0, utils_1.isCodeTransformation)(utils_1.CodeTransformation.JUST_IN_TIME)) {
        ...
        for (let j = 0; j < files.length; j++) {
            files[j] = (0, utils_1.transformFilePath)(files[j], outDir);
        }
    }
    for (let i = 0; i < files.length; i++) {
        ...
        if (info) {
            ...
            const defaultExport = (0, locter_1.getModuleExport)(fileExports);
            if (defaultExport &&
                typeorm_1.InstanceChecker.isDataSource(defaultExport.value)) {
