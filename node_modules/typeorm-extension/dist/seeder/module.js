...
async function prepareSeeder(options) {
    ...
    await (0, data_source_1.adjustFilePathsForDataSourceOptions)(options, {
    ...
    if (options.seeds) {
        ...
        if (seedFiles.length > 0) {
            ...
            for (let i = 0; i < seedFiles.length; i++) {
                const moduleExports = await (0, locter_1.load)(seedFiles[i]);
                const moduleDefault = (0, locter_1.getModuleExport)(moduleExports);
                if (moduleDefault.value) {
                    const item = moduleDefault.value;
