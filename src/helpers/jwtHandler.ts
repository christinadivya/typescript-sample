...
import { ILSRedisClient } from "../redis/redis";
...
export default class JwtHandler implements IJwtHandler {
  private redisClient: ILSRedisClient;
  private secretKey: string;
  public i18next: i18n;

  constructor(redisClient: ILSRedisClient, secretKey: string, i18next: i18n) {
  ...
  async verifyToken(token: string | undefined): Promise<JwtPayload> {
    ...
    if (!resp) {
      throw customExceptions.unAuthenticatedAccess(
        this.i18next.t("INVALID_AUTHORIZATION"),
        5
      );
    }
